...
class DealerBookingWindowPage extends StatefulWidget {
  final String dealerId;
  final String shopId;

  const DealerBookingWindowPage({
    Key? key,
  ...
  @override
  _DealerBookingWindowPageState createState() => _DealerBookingWindowPageState();
}

class _DealerBookingWindowPageState extends State<DealerBookingWindowPage> {
  ...
  void _openBookingWindow() {
    ...
    if (maxTokens <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Enter valid max tokens')),
      );
      return;
    }

    final newWindow = BookingWindow(
    ...
    setState(() {
      mockBookingWindows.add(newWindow);
    });

    BookingService.notifyUsers(newWindow);

    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Booking window opened! Users notified.')),
    );
